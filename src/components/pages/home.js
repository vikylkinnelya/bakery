...
class Home extends Component {
    ...
    render() {
        const { loading, menuType, setMenuType, addToCart, latestProducts, weekOffer, cart } = this.props
        ...
        const CarouselItem =
            <Row className='product-row'>
                {!loading && latestProducts != null && latestProducts.length > 0
                    && latestProducts.slice(this.state.startAt, this.state.endAt).map(menuItem => (
                        <ShopItem
                            key={menuItem.id}
                            menuItem={menuItem}
                            menuType={menuType}
